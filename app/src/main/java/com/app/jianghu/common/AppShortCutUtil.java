...
public class AppShortCutUtil {
	...
	public static void installRawShortCut(Context context, Class<?> clazz, boolean isShowNum, String num, boolean isStroke) {
		...
		// 快捷方式的图标
		if (isStroke) {
			shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON, generatorNumIcon4(context, ((BitmapDrawable) context.getResources()
									.getDrawable(R.mipmap.ic_launcher)).getBitmap(), isShowNum, num));
		} else {
			shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON, generatorNumIcon2(context, ((BitmapDrawable) context.getResources()
									.getDrawable(R.mipmap.ic_launcher)).getBitmap(), isShowNum, num));
		}
	 ...
	 * @param clazz 启动的activity
	 */
	public static void installRawShortCut(Context context, Class<?> clazz, String shortName, boolean isMain) {
        ...
        intent.putExtra(Intent.EXTRA_SHORTCUT_NAME, shortName);//快捷键名字可以任意，不过最好为app名称
        ...
        context.sendBroadcast(intent);//发送广播创建快捷键
    }
	 ...
	 * @return
	 */
	public static boolean isInstallShortcut(Context context, String shortName) {
        ...
        Cursor c = cr.query(CONTENT_URI, new String[] { "title", "iconResource" }, "title=?", new String[] { shortName }, null);
        ...
        c = cr.query(CONTENT_URI, new String[] { "title", "iconResource" }, "title=?", new String[] { shortName }, null);
        if (c != null && c.getCount() > 0) {
            isInstallShortcut = true;
        }
        if (isInstallShortcut) {
        ...
        //判断shared
        SharedPreferences spStartNum = context.getSharedPreferences(shortName, Context.MODE_PRIVATE);
        if (spStartNum.getInt(shortName, 0) <= 0) {
            spStartNum.edit().putInt(shortName, 1).apply();
        }else{
        	isInstallShortcut = true;
        }
        return isInstallShortcut;
	 ...
	 * 在小米应用图标的快捷方式上加数字<br>
	 * @param context
	 * @param num 显示的数字：大于99，为"99"，当为""时，不显示数字，相当于隐藏了)<br>
	...
	private static ShapeDrawable getDefaultBackground2(Context context) {
		...
		RoundRectShape rr = new RoundRectShape(outerR, null, null);
		ShapeDrawable drawable = new ShapeDrawable(rr);
		drawable.getPaint().setColor(DEFAULT_NUM_COLOR);// 设置填充颜色
		drawable.getPaint().setColor(DEFAULT_STROKE_COLOR);// 设置边框颜色
		drawable.getPaint().setStrokeWidth(distance);// 设置边框宽度
		return drawable;
	}
